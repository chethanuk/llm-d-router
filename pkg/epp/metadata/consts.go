...
const (
	...
	// FlowBandHeadroomRequestsHeaderKey is the response header carrying the remaining request capacity of the
	// flow control priority band the request occupied, as an integer request count. The reading is advisory and
	// shared: concurrent clients observe the same headroom and nothing is reserved for the client that read it.
	// It is absent when no reading is available, and zero means the band is full.
